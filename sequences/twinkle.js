...
exports.config = {
	description: "Twinkle - max of one random light turns off during any given interval"
};
...
function _generateAndSave(show, channels, duration) {
	...
	var theChannels = channels.split(',');
	theChannels.forEach(function (chan) {
		...
		for (startOffset; startOffset < stopAt; startOffset++) {
			// if all other channels (in our set) === 1 && random, canBeOff = true
			var canBeOff = true;
			var row = show.show[startOffset];
			theChannels.forEach(function (elem, index) {
				canBeOff = row[elem - 1] === 1 && canBeOff;
			});
			show.show[startOffset][chan - 1] = (Math.random() > 0.07 && canBeOff) ? 1 : 0;
		}
